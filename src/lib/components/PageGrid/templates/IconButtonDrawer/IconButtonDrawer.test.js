...
function setup() {
  const props = {
    _editable: '<!--#storyblok#{"name": "IconButtonDrawer", "space": "48408", "uid": "947be9f0-47c3-4315-a95a-550f0c560eb5", "id": "307934"}-->',
    anchor: 'left',
    elevation: 16,
    variant: 'temporary',
    button: [{
  ...
  const comp = mount(<IconButtonDrawer {...props} />);
  return { comp, props };
}

describe('<IconButtonDrawer />', () => {
  test('snapshot', () => {
    const { props } = setup();
    const tree = renderer.create(<IconButtonDrawer {...props} />);
    expect(tree).toMatchSnapshot();
  });

  it('should render IconButtonDrawer', () => {
    const { comp } = setup();
